  ...
  <DefaultLayout>
    <div class="p-4">
      ...
      <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold">Providers List</h2>
        <button
          @click="openAddModal"
        ...
        >
          ...
          >
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ provider.email }}</td>
...
import { ref } from 'vue'
import ProviderModal from '@/components/admin/Provider/ProviderModal.vue'
...
const addProvider = provider => {
  const newId =
    providers.value.length > 0
      ? Math.max(...providers.value.map(p => p.id)) + 1
      : 1
...
const handleProviderSubmit = formData => {
  if (selectedProvider.value) {
    // Handle edit
    updateProvider(selectedProvider.value.id, formData)
  } else {
    // Handle add
...
const deleteSelectedProvider = () => {
  if (selectedProvider.value) {
    deleteProvider(selectedProvider.value.id)
